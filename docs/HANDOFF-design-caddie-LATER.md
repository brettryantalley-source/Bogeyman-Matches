# Design brief (LATER): Caddie + Hole View screens

**Do not paste this into a Design thread yet.** The redesign starts with the ghost match only
(`HANDOFF-design.md`). This file keeps the Caddie content, states and real copy for when the
chosen direction is extended to the Caddie.
